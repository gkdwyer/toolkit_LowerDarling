...
```{r}
causal_ewr <- readRDS(file.path(ewr_results, "edited_CausalNetwork_2025-05-15.rds"))
```
...
```{r}
# Galen's good sequence:
#it made more sense to me to get to things like NF1 and then space than OB1_a and SF1_b and then space. #Since the env_objs are mapped to the hydrology at the gauge, seemed like that was the place to go to env_obj. I'm guessing, though, that #that level of specificity might not exist in the causal networks anymore
...
#have a look
```{r}
yearly <- read.csv(file.path(ewr_results, "module_output", "EWR", "hydrographs_LowerDarling_20250508", "yearly.csv"))
...
agged_data_1 <- read_and_agg(datpath = file.path(project_dir, "data", "module_output", "EWR", "hydrographs_LowerDarling_20250516"),
           type = 'achievement',
           ...
           keepAllPolys = FALSE,
           returnList = TRUE,
           savepath = file.path(agg_results, "basin_target"))

agged_data_2 <- read_and_agg(datpath =  file.path(project_dir, "data", "module_output", "EWR", "hydrographs_LowerDarling_20250516"), 
           type = 'achievement',
           geopath = bom_basin_gauges,
           causalpath = causal_ewr,
           groupers = c('scenario', 'SWSDLName', 'planning_unit_name', 'gauge'),
           aggCols = c("frequency_achieved",  "interevent_achieved", 'ewr_achieved'),
           #group_until = list(SWSDLName = is_notpoint, planning_unit_name = is_notpoint, gauge = is_notpoint),
           #pseudo_spatial = 'sdl_units',
